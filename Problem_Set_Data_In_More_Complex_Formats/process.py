...
def process_file(f):
    ...
    with open("{}/{}".format(datadir, f), "r") as html:
        ...
        classes = soup.find('table',{'class':'dataTDRight'})

        for tr in classes.find_all('tr'):
            td = tr.find_all('td')

            if td[1].text == 'TOTAL' or td[1].text == 'Month':
                continue
            row = {}
            row['courier'], row['airport'] = info['courier'], info['airport']

            row['year'] = int(td[0].text)
            row['month'] = int(td[1].text)
            row['flights'] = {'domestic': int(td[2].text.replace(',', '')),
