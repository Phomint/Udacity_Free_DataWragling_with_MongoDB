...
def process_file(filename, fields):
    ...
    with open(filename, "r") as f:
        ...
        for line in reader:
            ...
            line['name'] = line['rdf-schema#label'] if line['name'] == 'NULL' or not line['name'].isalnum() else line['name']
            todict['name'] = line['name']
            line['synonym'] = parse_array(line['synonym']) if line['synonym'] != 'NULL' else None
            todict['synonym'] = line['synonym']

            todict['classification'] = {}
            for field, value in fields.items():

                if field == 'name' or field == 'synonym':
                    continue

                line[field] = None if 'NULL' in line[field] else line[field].strip()

                if field == 'rdf-schema#label':
                    line[field] = line[field].split('(')[0].strip()

                if '_label' in field:
                    todict['classification'].update({value: line[field]})
                    continue

                todict[value] = line[field]
